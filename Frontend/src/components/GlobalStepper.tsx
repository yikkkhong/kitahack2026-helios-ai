...
const GlobalStepper: React.FC<GlobalStepperProps> = ({ currentStep, onBack, className = "" }) => {
  ...
  return (
    <div className={`flex items-center gap-3 pointer-events-auto ${className}`}>
      {/* A unified sci-fi style Back button */}
      <button 
        onClick={onBack} 
      ...
      {/* Minimalist breadcrumb progress bar (displayed only on medium and larger screens to prevent overcrowding on mobile devices). */}

...
const AssetizationPage = () => {
  ...
  const [panelCount, setPanelCount] = useState<number>(
    Number(blueprint?.technical_config?.panel_count) || 10
  );
  ...
  const [electricityRate, setElectricityRate] = useState<number>(
    Number(blueprint?.asset_potential?.eru_peg_rate_rm) || 0.5
  );

  const monthlyProductionKwh = panelCount * 45;
  const [monthlyConsumptionKwh, setMonthlyConsumptionKwh] = useState<number>(
    Math.floor(monthlyProductionKwh * 0.5)
  );

  const [totalCapTokens, setTotalCapTokens] = useState<number>(
    monthlyProductionKwh * 120
  );
  const INITIAL_GRANT =
    Number(blueprint?.asset_potential?.initial_grant_eru) ||
    totalCapTokens * 0.1;

  // --- 2. WALLET & VAULT STATES ---
  const [unlockedTokens, setUnlockedTokens] = useState<number>(INITIAL_GRANT);
  const [lockedTokens, setLockedTokens] = useState<number>(
    totalCapTokens - INITIAL_GRANT
  );
  ...
  const addLog = (
    message: string,
    type: 'gain' | 'loss' | 'neutral' = 'neutral'
  ) => {
    setLogs((prev: MarketLog[]) => [
      ...prev,
      { month: currentMonth, message, type },
    ]);
  };

  const visibleLogs = showAllLogs ? logs : logs.slice(-3);

  const [showTransitionTip, setShowTransitionTip] = useState(true);
  ...
  const handleFastForwardMonth = () => {
    ...
    if (surplusKwh > 0) {
      ...
      if (tokensToUnlock > 0) {
        ...
        addLog(
          `☀️ M${currentMonth}: Generated ${surplusKwh} kWh surplus. Unlocked ${tokensToUnlock} ERU.`,
          'gain'
        );
      } else {
        addLog(
          `☀️ M${currentMonth}: ${surplusKwh} kWh surplus, but 10-Yr Cap is fully unlocked!`,
          'neutral'
        );
      }
    } else {
      addLog(
        `☁️ M${currentMonth}: High consumption. No surplus to unlock.`,
        'loss'
      );
    }
  ...
  const handleAddPanels = () => {
    ...
    setLockedTokens(
      (prev: number) => prev + (additionalTokens10Years - immediateGrant)
    );

    addLog(
      `🔧 Installed ${panelsToAdd} panels. Cap increased. Unlocked ${immediateGrant} ERU immediately.`,
      'neutral'
    );
  };
  ...
  const handlePriceHike = () => {
    ...
    addLog(
      `📈 ALERT: National grid rate increased by RM 0.05. ERU value naturally appreciated!`,
      'gain'
    );
  };
  ...
  const handlePriceDrop = () => {
    ...
    setElectricityRate((prev: number) => Math.max(0.1, prev - 0.05));
    addLog(
      `📉 ALERT: National grid rate decreased by RM 0.05. ERU pegged value adjusted.`,
      'loss'
    );
  };
  ...
  const handleSell = (amount: number) => {
    ...
    addLog(
      `💰 Liquidated ${amount} ERU for RM ${revenue.toFixed(2)}`,
      'neutral'
    );
  };
  ...
  const generateChartData = () => {
    ...
    for (let m = 1; m <= 12; m++) {
      ...
      const aiSurplus = Math.max(
        0,
        currentProd - (monthlyConsumptionKwh + scenarioChangeKwh)
      );
        ...
        <div className="flex items-center gap-6">
          <div className="text-right">
            ...
            <div className="text-xl font-bold text-green-400 font-mono">
              RM {walletCash.toFixed(2)}
            </div>
          </div>
          <div className="w-px h-8 bg-white/20"></div>
          <div className="text-right">
            <div className="text-[10px] text-cyan-500 uppercase tracking-widest mb-1 drop-shadow-[0_0_5px_rgba(6,182,212,0.8)]">
              Total Net Worth
            </div>
            <div className="text-2xl font-bold text-white font-mono">
              RM {currentNetWorth.toFixed(2)}
            </div>
          </div>
        </div>
      </header>

      {/* Transition notification bar */}
      {showTransitionTip && (
        <div className="mx-6 mt-2 bg-blue-900/30 border border-blue-500/30 rounded-lg p-3 flex items-center justify-between animate-in fade-in">
          <div className="flex items-center gap-2 text-blue-300 text-sm">
            <Sparkles size={16} className="text-blue-400" />
            <span>
              Your solar configuration is ready. Now let's see how it becomes a
              digital asset — <span className="font-bold text-white">ERU</span>.
            </span>
          </div>
          <button
            onClick={() => setShowTransitionTip(false)}
            className="text-blue-400 hover:text-white p-1 rounded-full hover:bg-white/10 transition-colors"
          >
            <X size={18} />
          </button>
        </div>
          ...
          <div className="bg-zinc-900/80 backdrop-blur-xl border border-white/10 p-5 rounded-2xl flex justify-between items-center shrink-0">
            <div>
              <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wider flex items-center gap-1 mb-1">
                <DollarSign size={12} className="text-cyan-500" /> Utility Peg
                Rate
              </div>
              <span className="text-3xl font-black font-mono">
                RM {electricityRate.toFixed(2)}
              </span>
            </div>
            <div className="text-[9px] bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 px-2 py-1 rounded">
              FIXED RWA
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-blue-900/30 to-cyan-900/10 border border-blue-500/30 p-5 rounded-2xl shrink-0 flex flex-col">
            <div className="flex justify-between items-center mb-4">
              ...
              <span className="text-[10px] text-gray-400">
                10-Yr Cap: {Math.round(totalCapTokens).toLocaleString()}
              </span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div className="bg-black/40 p-3 rounded-xl border border-white/10">
                ...
                <div className="text-xl font-bold font-mono text-white">
                  {Math.round(unlockedTokens).toLocaleString()}
                </div>
              </div>
              <div className="bg-black/40 p-3 rounded-xl border border-white/10">
                <div className="flex items-center gap-1 text-gray-400 text-[10px] mb-1">
                ...
                <div className="text-xl font-bold font-mono text-gray-500">
                  {Math.round(lockedTokens).toLocaleString()}
                </div>
              </div>
            </div>
            ...
            <div className="mb-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  onClick={() => handleBuy(tradeAmount)}
                  className="bg-blue-600 hover:bg-blue-500 text-white py-2 rounded-lg font-bold text-xs transition shadow-[0_0_15px_rgba(37,130,246,0.3)]"
                >
                  Buy
                </button>
                <button
                  onClick={() => handleSell(tradeAmount)}
                  className="bg-zinc-800 hover:bg-zinc-700 border border-white/10 text-white py-2 rounded-lg font-bold text-xs transition"
                >
                  Liquidate
                </button>
              </div>
                ...
                logs
                  ...
                  .map((log: any, index: number) => {
                    ...
                    return (
                      <div
                        key={index}
                        className="text-[11px] text-gray-300 bg-black/40 border border-white/5 p-2.5 rounded-lg leading-relaxed"
                      >
                        {logText}
                      </div>
                    );
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      className={`flex-1 py-2 rounded-lg font-bold text-xs shadow-lg transition-transform hover:scale-[1.02] active:scale-95 ${
                        aiResponse.uiAction.type === 'BUY'
                          ? 'bg-blue-600 text-white'
                          : aiResponse.uiAction.type === 'LIQUIDATE'
      ...
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 animate-in slide-in-from-bottom-10">
        <div className="bg-black/90 backdrop-blur-2xl border border-white/20 p-2 rounded-full shadow-[0_0_50px_rgba(0,0,0,0.8)] flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-2 bg-zinc-800 hover:bg-zinc-700 text-white px-4 py-2.5 rounded-full text-xs font-bold transition"
          >
          ...
          <button
            onClick={handlePriceHike}
            className="flex items-center gap-2 bg-red-900/60 hover:bg-red-800/80 border border-red-500/50 text-red-200 px-4 py-2.5 rounded-full text-xs font-bold transition shadow-[0_0_10px_rgba(239,68,68,0.2)]"
          >
            <TrendingUp size={14} className="text-red-400" /> Trigger Grid Price
            Hike
          </button>

          <button
            onClick={handlePriceDrop}
            className="flex items-center gap-2 bg-blue-900/60 hover:bg-blue-800/80 border border-blue-500/50 text-blue-200 px-4 py-2.5 rounded-full text-xs font-bold transition shadow-[0_0_10px_rgba(59,130,246,0.2)]"
          >
                ...
                logs
                  ...
                  .map((log: any, index: number) => {
                    ...
                    const logText =
                      typeof log === 'string'
                        ? log
                        : log.message || log.text || log.action || '';

                    // 🎨 Pure visual intelligent classification logic: Analyze the extracted text.
                    const isBuy =
                      logText.toLowerCase().includes('buy') ||
                      logText.toLowerCase().includes('bought');
                    const isSell =
                      logText.toLowerCase().includes('liquidate') ||
                      logText.toLowerCase().includes('sell');

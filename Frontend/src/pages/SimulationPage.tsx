...
import { 
  Home, Car, Users, TrendingUp, PlayCircle, Sparkles, Activity, Play, Square, Volume2
} from 'lucide-react';
...
const SimulationPage = () => {
  ...
  const blueprint = location.state?.blueprintData 
      || JSON.parse(localStorage.getItem("step2_solar_blueprint") || "{}");
      
  const step1Concern = location.state?.concernData 
  ...
  const defaultRows = blueprint?.technical_config?.grid_layout?.rows || Math.floor(Math.sqrt(initialPanelCount));
  const defaultCols = blueprint?.technical_config?.grid_layout?.columns || Math.ceil(initialPanelCount / defaultRows);

  // Securely extract financial and ERU asset data
  const financialData = blueprint?.financial || { estimated_install_cost: 15000, yearly_savings_rm: 3000 };
  const capex = financialData.estimated_install_cost;
  const traditionalYearlySavings = financialData.yearly_savings_rm;
  
  const assetData = blueprint?.asset_potential || { total_eru_10yr: 54000, eru_peg_rate_rm: 0.5, initial_grant_eru: 16000 };
  const initialERU = assetData.initial_grant_eru;
  const eruPegRate = assetData.eru_peg_rate_rm;
  ...
  const handleExplainChart = async () => {
    ...
    try {
        ...
        console.error(error);
        setChartExplanation("AI is currently offline, but the chart clearly shows Helios ERU outperforming traditional solar.");
    } finally {
  ...
  const toggleSpeech = () => {
      if (isPlaying) {
          ...
          const preferredVoice = voices.find(v => v.name.includes("Google US English") || v.name.includes("Samantha")) || voices.find(v => v.lang === "en-US");
  ...
  // --- 3. The perfect ERU business logic: saving on electricity costs + converting surplus into assets.
  const getSimulationData = () => {
    const data = [];
    
    // Total annual power generation capacity = Number of tectonic plates * 540 kWh
    const totalCapacity_kWh_per_year = initialPanelCount * 540; 

    let myEruBalance = initialERU; 
    ...
    for (let i = 0; i <= simulationYear; i++) {
      if (i === 0) {
        data.push({ 
          name: `Year 0`, 
          Traditional: -capex, 
          HeliosAsset: (myEruBalance * eruPegRate) - capex, 
          CurrentERU: myEruBalance 
        });
        continue;
      }
      ...
      let heliosValue = accumulatedSavings + (myEruBalance * eruPegRate) - capex;

      data.push({
        name: `Year ${i}`,
        Traditional: Math.round(tradValue),
        HeliosAsset: Math.round(heliosValue),
        CurrentERU: Math.round(myEruBalance)
      });
    }
    return data;
  };
  ...
  const currentDataPoint = chartData[chartData.length - 1] || { CurrentERU: 0, HeliosAsset: 0, Traditional: 0 };
  // Define placement type
  const placementType = blueprint?.technical_config?.placement?.toUpperCase() || 'ROOFTOP';
  //const placementType = "BALCONY"; // For Testing Only: Force Virtual Mode

  const isBalcony = placementType.includes('BALCONY');
  const isVirtual = placementType.includes('VIRTUAL') || placementType.includes('BATTERY');
  ...
  // Platform length 
  const [platformWidth, setPlatformWidth] = useState<number>(recommendedPlatformWidth);
  
  // Number of boards (The quantity specified in Step 1 is used by default!)
  const [balconyPanelCount, setBalconyPanelCount] = useState<number>(initialPanelCount);
      ...
      try {
        ...
        const tileset = await Cesium.Cesium3DTileset.fromUrl(
          ...
          if (isBalcony) {
             ...
             const offset = new Cesium.HeadingPitchRange(0, Cesium.Math.toRadians(-20), 80);
            ...
            // virtual battery?
             ...
             const visualCenter = Cesium.Cartesian3.fromDegrees(lng, lat + 0.00015, altitude);
             viewer.camera.lookAt(visualCenter, new Cesium.HeadingPitchRange(0, Cesium.Math.toRadians(-35), 150));
          }
  ...
  useEffect(() => {
    ...
    if (isBalcony) {
        ...
        targetCameraCenter = Cesium.Cartesian3.fromDegrees(platformLng, platformLat, altitude);
        ...
        // 🟢 Upgrade: Make the platform rotate along with your Rotate slider!
        const rad = Cesium.Math.toRadians(-rotationHorizontal); 
        const platformHpr = new Cesium.HeadingPitchRoll(rad, 0, 0);
        const platformOrientation = Cesium.Transforms.headingPitchRollQuaternion(targetCameraCenter, platformHpr);
        ...
        // 🟢 Core Algorithm: Calculate the coordinates of the edge "directly in front".
        ...
        const dLng = Math.sin(rad) * forwardOffset / (111320 * Math.cos(platformLat * Math.PI / 180));

        // True mount point edge coordinates
        const edgeLat = platformLat + dLat;
        const edgeLng = platformLng + dLng;
        
        // 🟢 Core Algorithm: Reduce the board height! 0.2 meters lower than the platform.
        ...
        for (let i = 0; i < actualRenderCount; i++) {
            ...
            const offsetX = (i - (actualRenderCount - 1) / 2) * (singlePanelWidth + gap);
            
            const rightRad = rad + Math.PI / 2;
            const sLat = Math.cos(rightRad) * offsetX / 111320;
            const sLng = Math.sin(rightRad) * offsetX / (111320 * Math.cos(platformLat * Math.PI / 180));

            const panelPos = Cesium.Cartesian3.fromDegrees(edgeLng + sLng, edgeLat + sLat, hangHeight);
            ...
            const orientation = Cesium.Transforms.headingPitchRollQuaternion(panelPos, hpr);
        ...
        if (mode === 'EDIT') {            
             ...
             viewer.entities.add({
                position: targetCameraCenter,
                point: { 
                    pixelSize: 15, color: Cesium.Color.YELLOW, outlineColor: Cesium.Color.BLACK, outlineWidth: 2,
            ...
            viewer.entities.add({
                polyline: {
                    positions: [targetCameraCenter, Cesium.Cartesian3.fromDegrees(platformLng, platformLat, 0)],
                    width: 2,
        ...
        targetCameraCenter = Cesium.Cartesian3.fromDegrees(baseLng, baseLat, altitude);
        ...
        console.log("🏠 Roof mode detected (default). Loading Google 3D Tiles...");
        ...
        targetCameraCenter = Cesium.Cartesian3.fromDegrees(finalCenterLng, finalCenterLat, altitude);
    ...
    if (viewMode === "orbit" && targetCameraCenter) {
        ...
        const currentRange = Cesium.Cartesian3.distance(viewer.camera.positionWC, targetCameraCenter);
        
        // Tell the camera: No matter how the target moves, keep your eyes glued to it.
        viewer.camera.lookAt(targetCameraCenter, new Cesium.HeadingPitchRange(currentHeading, currentPitch, currentRange));
    }



  }, [altitude, gridRows, gridCols, nudgeLat, nudgeLng, rotationHorizontal, rotationVertical, pitch, mode, isLoading, viewMode, blueprint, platformWidth, balconyPanelCount]);
  ...
  const handleVerifyFeasibility = () => {
      ...
      setTimeout(() => {
          ...
          if (currentCount > initialPanelCount * 1.5) {
              setAiFeedback({ type: 'warning', msg: "Warning: Panel count significantly exceeds original budget and roof structural limit." });
          } 
          else if (currentCount < initialPanelCount * 0.5) {
              setAiFeedback({ type: 'warning', msg: "Warning: System size is too small to cover your current energy consumption." });
          } 
          else {
              setAiFeedback({ type: 'success', msg: "System Verified: Configuration is structurally safe and financially optimal." });
          }
          setIsChecking(false);
      }, 800); 
  };
    ...
    <div className="relative w-full h-screen bg-black overflow-hidden font-sans selection:bg-blue-500/30">
      ...
      <div className="absolute top-0 left-0 w-full p-4 flex justify-between items-center pointer-events-none z-50">
        ...
        <div className="pointer-events-auto bg-zinc-900/90 backdrop-blur-xl border border-white/10 p-1 rounded-full flex gap-1 shadow-2xl">
            ...
            <button onClick={() => setMode("SIMULATE")} className={`px-4 py-2 rounded-full text-sm font-bold flex items-center gap-2 transition-all ${mode === 'SIMULATE' ? 'bg-purple-600 text-white shadow-lg shadow-purple-500/50' : 'text-gray-400 hover:text-white'}`}>
      ...
      {mode === 'EDIT' && (
        ...
        {aiFeedback && (
            <div className="fixed top-24 left-1/2 -translate-x-1/2 z-[9999] pointer-events-auto animate-in slide-in-from-top-4 fade-in duration-300">
                <div className={`px-6 py-4 rounded-2xl border-2 backdrop-blur-2xl flex items-center gap-4 shadow-2xl ${
                    ...
                    {aiFeedback.type === 'success' ? <CheckCircle2 size={28} className="text-green-400 shrink-0"/> : <AlertTriangle size={28} className="text-yellow-400 shrink-0"/>}
                    <span className="font-mono text-sm font-bold max-w-sm leading-tight">{aiFeedback.msg}</span>
                    <button onClick={() => setAiFeedback(null)} className="ml-4 p-1 hover:bg-white/20 rounded-full transition-colors">
                        ✕
                    </button>
                </div>
            </div>
               ...
               <div className="bg-black/80 backdrop-blur-xl border border-cyan-500/50 rounded-2xl p-6 shadow-[0_0_30px_rgba(6,182,212,0.3)]">
                    ...
                    <div className="flex items-center gap-3 mb-6">
                        <div className="w-10 h-10 rounded-full bg-cyan-900/50 flex items-center justify-center border border-cyan-400/30">
                            <Zap size={20} className="text-cyan-400 fill-cyan-400 animate-pulse" />
                        </div>
                        <div>
                            <h3 className="text-white font-bold text-lg leading-tight">Helios Cloud</h3>
                            <p className="text-cyan-400 text-xs font-mono uppercase tracking-wider">Virtual Connection Active</p>
                        </div>
                    </div>
                    {/* Data overview */}
                    <div className="space-y-4">
                        <div className="bg-cyan-950/30 rounded-lg p-3 border border-cyan-800/30">
                            <div className="text-gray-400 text-xs mb-1">Subscription Plan</div>
                            <div className="text-white font-bold text-xl">5.4 kWp <span className="text-sm font-normal text-gray-500">Equivalent</span></div>
                        </div>
                        <div className="grid grid-cols-2 gap-3">
                            <div className="bg-cyan-950/30 rounded-lg p-3 border border-cyan-800/30">
                                <div className="text-gray-400 text-xs mb-1">Grid Offset</div>
                ...
                {/* In Virtual mode, only a simple AI button is needed; nothing else is required. */}
                 <div className="mt-4">
                    <button onClick={handleVerifyFeasibility} disabled={isChecking} className="w-full py-3 bg-cyan-900/50 border border-cyan-500/30 rounded-xl text-cyan-100 font-bold text-sm shadow-lg hover:bg-cyan-800/50 transition flex items-center justify-center gap-2">
          ...
          {!isVirtual && (
            <div className="absolute top-20 left-4 z-20 w-64 flex flex-col gap-3 animate-in slide-in-from-left duration-300">
                ...
                {isBalcony && (
                    <div className="bg-black/80 backdrop-blur-xl border border-cyan-500/30 rounded-2xl p-4 shadow-2xl">
                        ...
                        <div className="mb-4">
                            ...
                            <input type="range" min="2" max="20" step="0.1" value={platformWidth} onChange={(e) => setPlatformWidth(Number(e.target.value))} className="w-full h-1 bg-gray-700 rounded-lg accent-cyan-500"/>
                        </div>
                        ...
                        <div className="mb-4">
                            <div className="flex justify-between text-white text-sm mb-1">
                                ...
                                <span className="text-xs text-gray-400">Max: {Math.floor(platformWidth / PANEL_WIDTH)}</span>
                            </div>
                            <input 
                                type="range" 
                                min="1" 
                ...
                {isRooftop && (
                    <div className="bg-black/80 backdrop-blur-xl border border-white/10 rounded-2xl p-4 shadow-2xl">
                        ...
                        <div className="space-y-3">
                            ...
                            <div>
                                <div className="flex justify-between text-white text-sm mb-1">
                                    <span className="flex items-center gap-1"><RotateCw size={12}/> Slope</span>
                ...
                <div className="bg-black/80 backdrop-blur-xl border border-white/10 rounded-2xl p-4 shadow-2xl">
                    ...
                    >
                        {isPlacingMode ? "🛑 Click Map to Place" : (isBalcony ? "📍 Move Balcony (Tap)" : "📍 Move Panels (Tap)")}
                    </button>
                ...
                {/* --- C. ADD-ONS (Common for Rooftop & Balcony) --- */}
                ...
                <div className="relative mt-2">
                     ...
                     >
                        {isChecking ? <Loader2 className="animate-spin" size={16}/> : <CheckCircle2 size={16} />} 
             ...
             <div className="bg-black/80 backdrop-blur-2xl border border-white/10 rounded-3xl p-6 text-center text-white shadow-2xl">
                <div className="grid grid-cols-3 gap-4">
                    <div className="border-r border-white/10">
                        ...
                        <div className="text-2xl font-bold text-white">{(isVirtual ? 5.4 : totalYield).toFixed(1)} kWp</div>
                    </div>
                    <div className="border-r border-white/10">
                        <div className="text-gray-400 text-xs uppercase mb-1">Est. Savings</div>
                        <div className={`text-2xl font-bold transition-all ${hasBattery || isVirtual ? 'text-green-300 scale-110' : 'text-green-400'}`}>
                            RM {totalSavings.toLocaleString()}
                        </div>
                    </div>
                    <div>
                        <div className="text-gray-400 text-xs uppercase mb-1">{isVirtual ? "Status" : "Panels"}</div>
                        <div className="text-2xl font-bold text-blue-400">{isVirtual ? "Active" : currentCount}</div>
                    </div>
                </div>
             </div>
        </div>
      ...
      {mode === 'SIMULATE' && (
        <div className="absolute inset-0 z-40 flex flex-col justify-between pointer-events-none overflow-hidden animate-in fade-in duration-500">
          ...
          {eventTriggered && simulationYear >= 4 && (
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center justify-center z-0 animate-in zoom-in duration-300">
                ...
                <div className="bg-purple-900/60 backdrop-blur-xl border-2 border-purple-400 p-6 rounded-2xl shadow-[0_0_100px_rgba(168,85,247,0.4)] text-center transform scale-110 transition-all">
                    <Sparkles className="text-purple-300 mx-auto mb-2 animate-bounce" size={32} />
                    <h3 className="text-white text-2xl font-black uppercase mb-1">ERU Asset Deployed</h3>
                    <p className="text-purple-200 font-mono">Loss prevented. Portfolio Secured.</p>
                </div>
            </div>
          )}
          ...
          <div className="w-full px-6 pt-24 pb-6 flex justify-between items-start pointer-events-auto">
              ...
              <div className="relative">
                ...
                >
                  ...
                  <div className="flex items-center gap-3 z-10">
                    <div className={`p-2 rounded-lg ${eventTriggered ? 'bg-red-500/20' : 'bg-white/5'}`}>
                      {step1Concern === "Relocation" ? <Home size={16} /> : <Users size={16} />}
                    </div>
                    <div className="text-left">
                      <div className="text-[10px] text-gray-500 uppercase tracking-wider mb-0.5">Inject Uncertainty</div>
                      <div className="text-sm">
                        {step1Concern === "Relocation" ? "Relocate" : "Kids Move Out"}
                        {/* When not triggered, display the year as semi-transparent text next to it to save space. */}
                        {!eventTriggered && <span className="ml-1 opacity-40 font-normal">(Year {eventYear})</span>}
                      </div>
                    </div>
                  </div>
                  <div className={`w-3 h-3 rounded-full z-10 transition-colors ${eventTriggered ? 'bg-red-500 animate-pulse shadow-[0_0_10px_#ef4444]' : 'bg-zinc-600'}`}></div>
                </button>
                ...
                {eventTriggered && (
                  <div className="mt-2 flex items-center justify-between bg-red-950/40 border border-red-500/30 rounded-xl px-3 py-2 animate-in fade-in zoom-in-95 duration-200">
                    <span className="text-[10px] text-red-300/80 uppercase tracking-wider font-bold">Strike Year</span>
                    <div className="flex items-center gap-2 bg-black/50 rounded-lg p-1">
                      ...
                      <span className="text-white font-mono text-sm font-bold w-8 text-center">{eventYear}</span>
                      <button 
            ...
            <div className={`bg-black/60 backdrop-blur-xl border px-8 py-5 rounded-2xl flex flex-col items-end shadow-2xl transition-all duration-300 ${eventTriggered && simulationYear >= 4 ? 'border-purple-500 shadow-[0_0_40px_rgba(168,85,247,0.3)]' : 'border-white/10'}`}>
              <div className="text-xs text-gray-400 font-bold uppercase tracking-[0.2em] mb-2 flex items-center gap-2">
                <Activity size={14} className="text-purple-400"/> Live Asset Value
              </div>
              <div className="text-4xl font-mono font-black text-white tracking-tight drop-shadow-md">
                {currentDataPoint.CurrentERU.toLocaleString()} <span className="text-lg text-purple-400 font-bold">ERU</span>
              </div>
              ...
              <div className="text-sm text-gray-400 flex items-center gap-2">
                ...
                <span className="text-2xl font-bold text-green-400 font-mono">
                  RM {(currentDataPoint.CurrentERU * eruPegRate).toLocaleString()}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="w-full h-[35vh] min-h-[300px] px-8 pb-8 pt-12 pointer-events-auto relative">
            ...
            <div className="absolute top-0 right-12 z-20 flex flex-col items-end max-w-md">
                {!chartExplanation ? (
                    ...
                    >
                        {isExplaining ? <Loader2 className="animate-spin" size={16}/> : <Bot size={16} />}
                        {isExplaining ? "Analyzing Data..." : "Ask AI to Explain Chart"}
                    </button>
                ) : (
                    <div className="bg-black/80 backdrop-blur-2xl border border-cyan-500/50 p-5 rounded-2xl shadow-[0_0_30px_rgba(6,182,212,0.15)] animate-in slide-in-from-right-4 relative overflow-hidden">
                        ...
                        {isPlaying && <div className="absolute inset-0 bg-gradient-to-r from-transparent via-cyan-500/10 to-transparent animate-[shimmer_2s_infinite]"></div>}

                        <div className="flex items-center justify-between gap-6 mb-3 relative z-10">
                            <div className="flex items-center gap-2 text-cyan-400 text-xs font-bold uppercase tracking-wider">
                                {isPlaying ? <Volume2 size={16} className="animate-pulse" /> : <Bot size={16} />} 
                                AI Audio Briefing
                            </div>
                            <button onClick={() => { setChartExplanation(null); window.speechSynthesis.cancel(); setIsPlaying(false); }} className="text-gray-500 hover:text-white">✕</button>
                        </div>
                        ...
                        <div className="flex items-center justify-between border-t border-white/10 pt-3 relative z-10">
                            <div className="text-[10px] text-gray-500 flex items-center gap-1">
                                <Activity size={12} className={isPlaying ? "text-cyan-400 animate-bounce" : ""} /> 
                            ...
                            >
                                {isPlaying ? <><Square size={12} fill="currentColor" /> Stop</> : <><Play size={12} fill="currentColor" /> Play Audio</>}
                            </button>
                        </div>
                    </div>
                )}
            ...
            <div className="absolute top-0 left-12 flex gap-6 z-10 bg-black/40 backdrop-blur-md px-4 py-2 rounded-full border border-white/10 text-xs font-bold">
               <div className="flex items-center gap-2 text-gray-300">
                  ...
                  Traditional Solar <span className="opacity-50 font-normal">(Hardware Sunk Cost)</span>
               </div>
               <div className="flex items-center gap-2 text-white">
                  <div className="w-3 h-3 rounded-full bg-purple-500 shadow-[0_0_10px_#a855f7]"></div>
                  Helios ERU <span className="text-purple-300/50 font-normal">(Liquid Digital Asset)</span>
               </div>
            </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData} margin={{ top: 20, right: 0, left: 0, bottom: 0 }}>
                ...
                <XAxis dataKey="name" stroke="#888" fontSize={12} tickLine={false} axisLine={false} tickMargin={10} />
                ...
                tick={{ fill: '#9ca3af', style: { textShadow: '0px 2px 4px rgba(0,0,0,1)' } }}
                tickFormatter={(val) => `RM ${val.toLocaleString()}`} 
                width={80} 
                />
                
                <Tooltip 
                  contentStyle={{ backgroundColor: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(10px)', borderColor: 'rgba(168,85,247,0.3)', borderRadius: '12px', padding: '12px' }}
                  itemStyle={{ color: '#fff', fontWeight: 'bold', fontSize: '14px' }}
                  labelStyle={{ color: '#888', marginBottom: '8px', fontSize: '12px', textTransform: 'uppercase' }}
                  formatter={(value: any, name: any) => {
                     // Make the name Tooltip look better
                     const label = name === "Traditional" ? "Traditional Value" : "Helios ERU Value";
                     return [`RM ${Number(value || 0).toLocaleString()}`, label];
                  }}
                />

                {/* Traditional solar power lines (in red to indicate losses during a crisis) */}
                <Area type="monotone" dataKey="Traditional" name="Traditional" stroke="#ef4444" strokeWidth={2} fill="url(#colorTrad)" animationDuration={1000} />
                
                {/* Helios ERU line (purple, shimmering) */}
                <Area type="monotone" dataKey="HeliosAsset" name="HeliosAsset" stroke="#d8b4fe" strokeWidth={4} fill="url(#colorHelios)" animationDuration={1000} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
